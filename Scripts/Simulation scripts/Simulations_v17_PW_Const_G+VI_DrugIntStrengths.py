...
sys.path.append(dir_path)
from Functions import set_z, scale_intradius
import pickle
import mesa
import numpy as np
import pandas as pd
...
f_conc = [x[2][0]/6 for x in ab_conc_range_alldrugs]
...
z_range = [-0.9999, np.nan] # range for MIC changes, TRY TO AVOID UPPER LIMIT
y_range = [-0.9999, 3.5] # range for MIC changes, TRY TO AVOID UPPER LIMIT
    ...
    for m_type,n_steps_r in zip(move_type,n_steps_move_type):
    	for n_steps in n_steps_r:
            for all_interactions in all_interactions_range:
                ...
                for MIC_change in MIC_change_range_original:
                    G_change = [set_z(MIC_change,n,g,all_species[0][0][2],f) for g,n,f in zip(drugs[0],drugs[1],f_conc)][0]
                    G_change = scale_intradius(G_change,1,int_radius)
                    z_range = [-0.9999, [set_z(y_range[1],n,g,all_species[0][0][2],f) for g,n,f in zip(drugs[0],drugs[1],f_conc)][0]] # range for MIC changes, TRY TO AVOID UPPER LIMIT
                    for ab_conc,ab_conc_time in zip(ab_conc_range,ab_conc_time_range):
                        ...
                        for i in range(its):
                            ...
                            db_model[(results_df['drugs'].loc[0][1][0],
                                ...
                                round(results_df['G_change'].loc[0],ndigits=6),
...
folder_name="Simulations_v17_PW_Const_G+VI_DrugIntStrengths"
